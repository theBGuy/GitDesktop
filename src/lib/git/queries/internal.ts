 ...
 * boards stay marked stale for the next natural read. A write that patches WITHOUT
 * marking (writeThroughBoards' `markStale: false`) leaves no mark of its own but
 * preserves the ones it found, so a mark laid here still reaches its refetch. A lone
 * write here sees zero and refetches immediately, exactly as before.
 *
