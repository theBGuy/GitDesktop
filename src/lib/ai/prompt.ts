...
- XSS: where a framework or template engine escapes by default (React/JSX, Vue, Svelte, Angular templates, Django templates, Handlebars, Rails/ActionView ERB, and Jinja2 only where autoescaping is actually on — Flask's HTML templates, \`select_autoescape()\`, or an explicit \`autoescape=True\`), report only via an explicit escape hatch — \`dangerouslySetInnerHTML\`, \`v-html\`, \`{@html}\`, \`bypassSecurityTrust*\`, \`|safe\`/\`mark_safe\`/\`{% autoescape off %}\`, \`raw\`/\`html_safe\`, triple-stash \`{{{ }}}\`, direct \`innerHTML\`/\`document.write\`. Where markup is assembled by string concatenation or hand-rolled templating — including a bare \`jinja2.Environment()\` (autoescape defaults to off) or plain \`ERB.new\` outside Rails — ordinary XSS rules apply in full.
