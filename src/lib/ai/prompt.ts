...
- Memory safety (buffer overflow, use-after-free, pointer arithmetic): a real class in C, C++, Objective-C, \`unsafe\` Rust blocks, Go's \`unsafe\`/\`unsafe.Pointer\`, cgo, unsafe C#, and raw FFI — report it there. Not reachable in ordinary memory-safe/managed code — don't report it there.
- XSS: where a framework or template engine escapes by default (e.g. React/JSX, Vue, Svelte, Angular templates, Django templates, Handlebars, Go \`html/template\`, Rails/ActionView ERB, and Jinja2 only where autoescaping is actually on — Flask's HTML templates, \`select_autoescape()\`, or an explicit \`autoescape=True\`), report only via an explicit escape hatch — \`dangerouslySetInnerHTML\`, \`v-html\`, \`{@html}\`, \`bypassSecurityTrust*\`, \`|safe\`/\`mark_safe\`/\`{% autoescape off %}\`, \`raw\`/\`html_safe\`, \`template.HTML\`/\`template.JS\`, triple-stash \`{{{ }}}\`, direct \`innerHTML\`/\`document.write\`. Where markup is assembled by string concatenation or hand-rolled templating — including a bare \`jinja2.Environment()\` (autoescape defaults to off) or plain \`ERB.new\` outside Rails — or by an engine whose escaping you cannot establish, ordinary XSS rules apply in full.
