...
- XSS: where a framework or template engine escapes by default (React/JSX, Vue, Svelte, Angular templates, Jinja2/Django, ERB, Handlebars), report only via an explicit escape hatch — \`dangerouslySetInnerHTML\`, \`v-html\`, \`{@html}\`, \`bypassSecurityTrust*\`, \`|safe\`/\`mark_safe\`, \`raw\`/\`html_safe\`, triple-stash \`{{{ }}}\`, direct \`innerHTML\`/\`document.write\`. Where markup is assembled by string concatenation or hand-rolled templating, ordinary XSS rules apply in full.
