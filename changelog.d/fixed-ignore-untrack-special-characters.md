- **File actions and views now use exactly the file you picked when its path
  ...
  fold in a neighbour's commits and hunks. *Ignore*, *Untrack* and *Exclude from
  AI* also write a working pattern for a name that ends in a space, which
  previously matched a different file and left the one you picked alone.
