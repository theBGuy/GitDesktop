- **Set an issue or pull request's GitHub Projects fields without leaving it.** The
  **Project fields** row is now a trigger: it opens one popup holding every field you can
  ...
  drafts your edits and writes them when it closes, one write per board. **Edit project
  fields…** in the command palette opens it without the mouse. GitHub only, and it uses the
  same `project` sign-in scope the Projects picker already needs.
