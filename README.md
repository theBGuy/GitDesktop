...
- **Open, edit, merge**: review, comment, approve, edit, and merge (merge &
  ...
  (Settings → General). A new PR is visible in the list from the moment you
  start creating it: a **pending entry** holds its place with the title,
  branches and draft state, picks up the number as the PR opens, and hands
  over to the real row once the list catches up.
